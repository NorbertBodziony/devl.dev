        ...
        <div className="mt-10 rounded-xl border border-border/60 bg-background/40 p-3 shadow-sm focus-within:border-foreground/40">
          <div className="flex items-center justify-between border-b border-border/40 pb-2">
            <div className="flex items-center gap-0.5">
              {TOOLS.map((t, i) => (<button key={i} type="button" title={t.shortcut ? `${t.label} (${t.shortcut})` : t.label} className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-foreground/[0.05] hover:text-foreground">
                  <t.Icon className="size-3.5"/>
                </button>))}
              <span className="mx-1 h-4 w-px bg-border/60"/>
              <button type="button" title="Attach" className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-foreground/[0.05] hover:text-foreground">
                <PaperclipIcon className="size-3.5"/>
              </button>
              <button type="button" title="Image" className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-foreground/[0.05] hover:text-foreground">
                <ImageIcon className="size-3.5"/>
              </button>
              <button type="button" title="Mention" className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-foreground/[0.05] hover:text-foreground">
                <AtSignIcon className="size-3.5"/>
              </button>
              <button type="button" title="Emoji" className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-foreground/[0.05] hover:text-foreground">
          ...
          <textarea value={draft()} onChange={(e) => setDraft(e.target.value)} rows={5} placeholder="Reply to the thread…" className="mt-2 w-full resize-none bg-transparent px-1 text-sm leading-relaxed outline-none placeholder:text-muted-foreground"/>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-md border border-border/60 bg-background/40 px-2 py-1 text-xs">
              ...
              <button type="button" className="rounded text-muted-foreground/60 transition-colors hover:text-foreground" aria-label="Remove">
                ×
              </button>
            </div>
          </div>
