...
export function Toggle(props: any) {
  ...
  const [pressed, setPressed] = createSignal(
    Boolean(local.pressed ?? local.defaultPressed),
  );

  createEffect(() => {
    if (local.pressed !== undefined) setPressed(Boolean(local.pressed));
  });
  ...
  return (
    <Primitive
      ...
      type="button"
      base={cn(
        toggleVariants({ size: local.size ?? "default", variant: local.variant ?? "default" }),
        local.className,
        local.class,
      )}
