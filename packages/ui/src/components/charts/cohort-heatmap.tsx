...
export function CohortHeatmap(props: CohortHeatmapProps) {
  ...
  return (
    <div
      class={cn(
        "overflow-x-auto rounded-xl border border-border/60 bg-background/40 p-5",
        props.class,
      )}
    >
                ...
                return (
                  <td class="p-0.5">
                    {value === null ? (
                      ...
                      <HeatmapCell highlight={index === 0} value={value} />
                    )}
                  </td>
                );
              })}
      ...
      <div class="mt-5 flex items-center gap-3 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.2em]">
        ...
        <span>More</span>
        {props.averageLabel ? (
          <span class="ml-auto">{props.averageLabel}</span>
        ) : null}
      </div>
    </div>
  );
...
function HeatmapCell(props: { highlight?: boolean; value: number }) {
