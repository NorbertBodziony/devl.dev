...
function buildRrule(r: Rule): {
    ...
    if (r.interval > 1)
        tokens.push({ k: "INTERVAL", v: String(r.interval) });
...
function describeRule(r: Rule): string {
    const intervalWord = (singular: string, plural: string) => r.interval > 1 ? `${r.interval} ${plural}` : singular;
    let main = "";
    if (r.freq === "DAILY")
    ...
    if (r.freq === "MONTHLY") {
        if (r.monthlyMode === "day") {
            ...
            const pos = POSITIONS.find((p) => p.value === r.bysetpos)?.label ?? "";
            const days = r.bydayPositional.length === 5 &&
                r.bydayPositional.every((d) => ["MO", "TU", "WE", "TH", "FR"].includes(d))
                ? "weekday"
    ...
    if (r.end.type === "count")
        main += `, for ${r.end.count} occurrences`;
    return main + ".";
}
...
function expandOccurrences(rule: Rule, start: Date, max: number): Date[] {
    ...
    const limit = (d: Date) => {
        if (rule().end.type === "until")
            return d <= new Date(rule().end.date);
        return true;
    };
    const cap = rule().end.type === "count" ? Math.min(max, rule().end.count) : max;
    if (rule().freq === "DAILY") {
    ...
    if (rule().freq === "MONTHLY") {
        ...
        while (out.length < cap && safety++ < 200) {
            ...
            else if (rule().bydayPositional.length &&
                rule().bysetpos !== null) {
                const matches: Date[] = [];
                for (let day = 1; day <= last; day++) {
                    const d = new Date(y, mo, day);
                    if (rule().bydayPositional.some((wd) => DAY_INDEX[wd] === d.getDay()))
                        matches.push(d);
                }
                const idx = rule().bysetpos > 0
                    ? rule().bysetpos - 1
    ...
    return (<div className="min-h-svh bg-muted/30 px-6 py-8">
      <div className="mx-auto max-w-5xl space-y-4">
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-background shadow-sm">
          <Templates current={built().raw} onPick={(t) => setRule((r) => ({ ...DEFAULT_RULE, ...r, ...t }))}/>
          <div className="grid grid-cols-1 divide-y divide-border/60 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
            ...
            <Preview rule={rule()} tokens={built().tokens} raw={built().raw} description={description()} occurrences={next8} allOccurrences={occurrences()} monthOffset={monthOffset()} setMonthOffset={setMonthOffset}/>
          </div>
...
function Templates({ current, onPick, }: {
    current: string;
    onPick: (rule: Partial<Rule>) => void;
}) {
    return (<div className="flex items-center gap-2 overflow-x-auto border-border/60 border-b bg-background px-5 py-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        {TEMPLATES.map((t) => {
            ...
            return (<button key={t.label} type="button" onClick={() => onPick(t.rule)} className={"rounded-md border px-2.5 py-0.5 text-[12px] transition-colors " +
                    (isActive
    ...
    return (<div className="space-y-5 px-5 py-5">
      <Row label="FREQ">
        <div className="flex items-center rounded-md border border-border/70 bg-background p-0.5">
          {FREQS.map((f) => (<button key={f} type="button" onClick={() => apply({ freq: f })} className={"rounded px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.15em] " +
                (rule().freq === f
      ...
      <Row label="INTERVAL">
        <div className="flex items-center gap-2">
          <Input type="number" value={rule().interval} onChange={(e) => apply({ interval: Math.max(1, Number(e.target.value || 1)) })} className="w-16" min={1}/>
      ...
      {rule().freq === "WEEKLY" && (<Row label="BYDAY">
          <div className="flex items-center gap-1">
            {WEEKDAYS_DISPLAY.map((d) => {
                const active = rule().byday.includes(d.key);
                return (<button key={d.key} type="button" onClick={() => apply({
                        byday: active
                            ? rule().byday.filter((x) => x !== d.key)
                            : [...rule().byday, d.key],
                    })} className={"grid size-8 place-items-center rounded-md font-mono text-xs transition-colors " +
                        (active
      ...
      {rule().freq === "MONTHLY" && (<>
          <Row label="ON">
            <div className="flex items-center rounded-md border border-border/70 bg-background p-0.5">
              <button type="button" onClick={() => apply({ monthlyMode: "day" })} className={"rounded px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.15em] " +
                (rule().monthlyMode === "day"
                    ? "bg-foreground/[0.08] text-foreground"
                    : "text-muted-foreground hover:text-foreground")}>
                Day of month
              </button>
              <button type="button" onClick={() => apply({ monthlyMode: "weekday" })} className={"rounded px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.15em] " +
          ...
          {rule().monthlyMode === "day" && (<Row label="BYMONTHDAY">
              <DayOfMonthGrid selected={rule().bymonthday} onChange={(bymonthday) => apply({ bymonthday })}/>
            </Row>)}

          {rule().monthlyMode === "weekday" && (<>
              <Row label="BYSETPOS">
                <div className="flex items-center rounded-md border border-border/70 bg-background p-0.5">
                  {POSITIONS.map((p) => (<button key={p.value} type="button" onClick={() => apply({ bysetpos: p.value })} className={"rounded px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.15em] " +
      ...
      <Row label="ENDS">
        <div className="flex flex-col gap-2">
          <EndOption label="Never" checked={rule().end.type === "never"} onSelect={() => apply({ end: { type: "never" } })}/>
          <EndOption label="On a date" checked={rule().end.type === "until"} onSelect={() => apply({ end: { type: "until", date: "2026-07-26" } })} trailing={rule().end.type === "until" ? (<button type="button" className="ml-auto inline-flex items-center gap-2 rounded-md border border-border/70 bg-background px-2 py-1 font-mono text-[11px]">
          ...
          <EndOption label="After N times" checked={rule().end.type === "count"} onSelect={() => apply({ end: { type: "count", count: 12 } })} trailing={rule().end.type === "count" ? (<Input type="number" value={rule().end.count} onChange={(e) => apply({
                end: {
                    type: "count",
                    count: Math.max(1, Number(e.target.value || 1)),
                },
            })} className="ml-auto w-20"/>) : null}/>
        </div>
      </Row>
    </div>);
}
...
function EndOption({ label, checked, onSelect, trailing, }: {
    label: string;
    checked: boolean;
    onSelect: () => void;
...
function DayOfMonthGrid({ selected, onChange, }: {
    selected: number[];
    onChange: (next: number[]) => void;
}) {
    const toggle = (n: number) => {
        onChange(selected.includes(n)
    ...
    return (<div className="space-y-1.5">
      <div className="grid grid-cols-7 gap-1">
        {Array.from({ length: 31 }, (_, i) => i + 1).map((n) => {
            ...
            return (<button key={n} type="button" onClick={() => toggle(n)} className={"grid h-7 place-items-center rounded font-mono text-[11px] tabular-nums " +
                    (active
                        ? "bg-foreground text-background"
        ...
        <button type="button" onClick={() => toggle(-1)} className={"col-span-3 grid h-7 place-items-center rounded font-mono text-[10px] uppercase tracking-[0.2em] " +
            (selected.includes(-1)
                ? "bg-foreground text-background"
                : "border border-border/60 text-foreground/80 hover:border-foreground/40")}>
          Last day
...
function Preview({ tokens, raw, description, occurrences, allOccurrences, monthOffset, setMonthOffset, }: {
    rule: Rule;
    tokens: {
        k: string;
        v: string;
    ...
    return (<div className="space-y-5 bg-muted/10 px-5 py-5">
      ...
      <MiniCalendar occurrences={allOccurrences} offset={monthOffset()} setOffset={setMonthOffset}/>
      ...
      <div>
        ...
        <ul className="mt-2 divide-y divide-border/40 overflow-hidden rounded-lg border border-border/60 bg-background">
          {occurrences().length === 0 ? (<li className="px-3 py-4 text-center text-[12px] text-muted-foreground">
              No occurrences match this rule.
            </li>) : (occurrences().map((d, i) => {
            const days = Math.round((d.getTime() - START.getTime()) / 86400000);
            return (<li key={i} className="flex items-baseline justify-between px-3 py-1.5 font-mono text-[12px]">
...
function MiniCalendar({ occurrences, offset, setOffset, }: {
    occurrences: Date[];
    offset: number;
    ...
    const today = START;
    const hits = new Set(occurrences().filter((d) => d.getFullYear() === y && d.getMonth() === mo)
        .map((d) => d.getDate()));
    ...
    while (cells.length % 7 !== 0)
        cells.push(null);
    return (<div className="rounded-lg border border-border/60 bg-background p-3">
      <div className="mb-2 flex items-center justify-between">
        <button type="button" onClick={() => setOffset(offset - 1)} className="grid size-6 place-items-center rounded text-muted-foreground hover:bg-muted hover:text-foreground">
      ...
      <div className="grid grid-cols-7 gap-y-1 text-center">
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (<div key={i} className="font-mono text-[9px] text-muted-foreground uppercase tracking-[0.2em]">
            {d}
          </div>))}
        {cells.map((c, i) => {
            if (c === null)
            ...
            return (<div key={i} className="grid place-items-center">
              <div className={"grid size-7 place-items-center rounded-md font-mono text-[11px] tabular-nums " +
                    (isHit
                        ? "bg-foreground text-background"
                        : isToday
