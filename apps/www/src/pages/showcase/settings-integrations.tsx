      ...
      <div className="mx-auto max-w-5xl px-8 py-12">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="flex flex-1 items-center gap-2 rounded-md border border-border/70 bg-background/40 px-3">
            ...
            <input value={query()} onChange={(e) => setQuery(e.target.value)} placeholder="Find an integration" className="h-9 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground"/>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {FILTERS.map((f) => (<button key={f} type="button" onClick={() => setFilter(f)} className={`rounded-md px-2.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.25em] transition-colors ${filter() === f
        ...
        <ul className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((i) => (<IntegrationCard key={i.id} integration={i}/>))}
        </ul>

        {visible.length === 0 ? (<div className="mt-8 rounded-xl border border-dashed border-border/70 bg-background/30 px-6 py-12 text-center">
...
function IntegrationCard({ integration }: {
    ...
    return (<li className="group flex flex-col gap-3 rounded-xl border border-border/60 bg-background/40 p-4 transition-colors hover:border-foreground/30">
      <div className="flex items-start justify-between">
        <div className={`flex size-10 items-center justify-center rounded-lg font-medium text-[12px] ${integration.glyphTone}`}>
          {integration.glyph}
        </div>
        {integration.badge ? (<span className="rounded bg-foreground/[0.06] px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.2em] text-muted-foreground">
            {integration.badge}
          </span>) : null}
      </div>
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          {integration.connected ? (<span className="inline-flex items-center gap-1 rounded bg-emerald-500/10 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-400">
      ...
      <div className="mt-auto flex items-center gap-2 pt-1">
        <Button size="sm" variant={integration.connected ? "outline" : "default"} type="button" className="flex-1">
          {integration.connected ? "Configure" : "Connect"}
        </Button>
        <button type="button" className="inline-flex items-center justify-center rounded-md border border-border/60 bg-background/40 p-2 text-muted-foreground transition-colors hover:text-foreground" aria-label="Open docs">
