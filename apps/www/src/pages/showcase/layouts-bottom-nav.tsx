...
function PhoneFrame({ activeTab, setActiveTab, }: {
    activeTab: TabKey;
    setActiveTab: (tab: TabKey) => void;
}) {
    return (<div className="relative w-[380px] h-[760px] rounded-[40px] border-8 border-foreground/15 bg-background shadow-2xl overflow-hidden">
...
function StatusBar() {
    return (<div className="h-10 px-6 flex items-center justify-between text-xs font-medium pt-3">
      <span>9:41</span>
...
function AppHeader({ title }: {
    ...
    return (<div className="px-5 py-3 flex items-center justify-between border-b border-border/60">
...
function HomeFeed() {
    return (<div className="flex flex-col gap-3">
      {FEED.map((item) => (<article key={item.name} className="rounded-2xl border border-border/60 bg-card p-3">
          ...
          <div className={`mt-2.5 h-24 w-full rounded-xl bg-gradient-to-br ${item.tone}`}/>
...
function DiscoverGrid() {
    return (<div className="grid grid-cols-2 gap-3">
      {DISCOVER.map((item) => (<div key={item.title} className="flex flex-col gap-1.5">
          <div className={`aspect-square w-full rounded-2xl bg-gradient-to-br ${item.tone}`}/>
...
function CreatePrompt() {
    return (<div className="flex h-full items-center justify-center py-10">
...
function AlertSection({ label, items, }: {
    ...
    return (<section className="flex flex-col gap-2">
      ...
      <div className="flex flex-col rounded-2xl border border-border/60 bg-card divide-y divide-border/60">
        {items.map((item) => (<div key={item.name + item.time} className="flex items-start gap-2.5 p-3">
...
function ProfileView() {
    return (<div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col rounded-2xl border border-border/60 bg-card divide-y divide-border/60">
        {PROFILE_ACTIONS.map((action) => (<button key={action.label} type="button" className="flex items-center gap-3 p-3.5 text-left hover:bg-muted/40 transition-colors first:rounded-t-2xl last:rounded-b-2xl">
...
function ProfileStat({ label, value }: {
    ...
    return (<div className="flex-1 flex flex-col items-center justify-center py-3">
...
function BottomNav({ activeTab, setActiveTab, }: {
    ...
    return (<nav className="h-[72px] border-t border-border/60 bg-background flex items-stretch justify-around relative">
      {TABS.map((tab) => {
            const isActive = activeTab === tab.key;
            const isCenter = tab.key === "create";
            if (isCenter) {
                return (<button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)} aria-label={tab.label} className="flex flex-col items-center justify-center gap-0.5 flex-1 py-2 cursor-pointer">
              ...
              <span className={`text-[10px] -mt-2 ${isActive ? "text-foreground" : "text-muted-foreground"}`}>
                {tab.label}
              </span>
            </button>);
            }
            const Icon = tab.icon;
            return (<button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)} aria-label={tab.label} aria-current={isActive ? "page" : undefined} className={`relative flex flex-col items-center justify-center gap-0.5 flex-1 py-2 cursor-pointer transition-colors ${isActive ? "text-foreground" : "text-muted-foreground"}`}>
            {isActive ? (<span className="absolute top-0 left-1/2 -translate-x-1/2 h-0.5 w-6 rounded-full bg-foreground"/>) : null}
            <span className="relative">
              <Icon className="size-5" strokeWidth={isActive ? 2.4 : 2}/>
              {tab.badge ? (<span className="absolute -top-1.5 -right-2 min-w-[16px] h-4 px-1 rounded-full bg-foreground text-background text-[9px] font-medium flex items-center justify-center">
                  {tab.badge}
                </span>) : null}
            </span>
