        ...
        <div className="mt-10 rounded-2xl border border-border/60 bg-background/40 p-8">
          <div className="flex items-baseline justify-between">
            <div>
              ...
              {totalAnnual ? (<p className="mt-1 font-mono text-[10px] text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.25em]">
                  ${totalAnnual.toLocaleString()} billed annually · save 20%
                </p>) : (<p className="mt-1 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em]">
                  Billed monthly
                </p>)}
            </div>
            <div className="inline-flex rounded-full border border-border/60 bg-background/40 p-0.5 font-mono text-[11px] uppercase tracking-[0.2em]">
              <button type="button" onClick={() => setAnnual(false)} className={`rounded-full px-3 py-1.5 transition-colors ${!annual() ? "bg-foreground text-background" : "text-muted-foreground"}`}>
                Monthly
              </button>
              <button type="button" onClick={() => setAnnual(true)} className={`rounded-full px-3 py-1.5 transition-colors ${annual() ? "bg-foreground text-background" : "text-muted-foreground"}`}>
                Yearly
              </button>
            </div>
          </div>
          ...
          <div className="mt-10">
            <div className="flex items-baseline justify-between">
              <label htmlFor="seats" className="font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em]">
                Seats
              </label>
            ...
            <input id="seats" type="range" min={1} max={50} value={seats()} onChange={(e) => setSeats(Number(e.target.value))} className="mt-3 w-full accent-foreground"/>
          ...
          <ul className="mt-8 grid grid-cols-2 gap-2.5">
            ...
            "Custom roles",
        ].map((f) => (<li key={f} className="flex items-start gap-2 text-sm">
