...
const isAutoJoin = (email: string) => email.toLowerCase().endsWith(`@${WORKSPACE_DOMAIN}`);
...
export function FormsInviteTeammatesShowcasePage() {
    ...
    const setRole = (id: number, role: Role) => setChips((prev) => prev.map((c) => (c.id === id ? { ...c, role } : c)));
        ...
        <form onSubmit={onSubmit} className="mt-8 rounded-xl border border-border/60 bg-background/40 p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select items={ROLES} value={defaultRole()} onValueChange={(v) => setDefaultRole(v as Role)}>
          ...
          <button type="button" onClick={() => inputRef.current?.focus()} className="mt-3 flex w-full cursor-text flex-wrap items-center gap-1.5 rounded-lg border border-input bg-background p-2 text-left transition-shadow focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/24">
            {chips().map((chip) => (<ChipPill key={chip.id} chip={chip} onRemove={() => removeChip(chip.id)} onRole={(role) => setRole(chip.id, role)}/>))}
            <input ref={inputRef} type="email" value={draft()} onChange={(e) => setDraft(e.target.value)} onKeyDown={onKeyDown} onPaste={onPaste} onBlur={onBlur} placeholder={chips().length === 0
            ? "name@example.com, another@example.com…"
            : "Add another"} className="min-w-[12ch] flex-1 bg-transparent px-1.5 py-1 text-sm outline-none placeholder:text-muted-foreground"/>
          </button>
          ...
          <div className="mt-2 flex items-center gap-3 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em]">
            ...
            {stats().autojoin > 0 && (<span className="flex items-center gap-1">
                <SparklesIcon className="size-3"/>
                {stats().autojoin} auto-join
              </span>)}
            {stats().invalid > 0 && (<span className="text-destructive">
                {stats().invalid} invalid
              </span>)}
          </div>

          {showMessage() ? (<div className="mt-5 flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <Label htmlFor="invite-message" className="font-medium text-sm">
                  Personal note
                </Label>
                <button type="button" onClick={() => setShowMessage(false)} className="font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em] hover:text-foreground">
                  Hide
                </button>
              </div>
              <Textarea id="invite-message" rows={3} value={message()} onChange={(e) => setMessage(e.target.value)} placeholder="Add a quick hello so they know what they're joining."/>
            </div>) : (<button type="button" onClick={() => setShowMessage(true)} className="mt-4 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em] hover:text-foreground">
              + Add a personal note
            </button>)}
...
function ChipPill({ chip, onRemove, onRole, }: {
    chip: Chip;
    onRemove: () => void;
    ...
    return (<span className={[
      ...
      {autoJoin ? (<span className="flex items-center gap-0.5 font-mono text-[9px] text-emerald-700 uppercase tracking-[0.2em] dark:text-emerald-400">
          ...
          <PopoverPopup className="w-56 p-1">
            {ROLES.map((r) => {
                ...
                return (<button key={r.value} type="button" onClick={() => onRole(r.value)} className="flex w-full items-start gap-2 rounded-md px-2 py-1.5 text-left text-sm hover:bg-foreground/[0.04]">
                  <CheckIcon className={[
        ...
        </Popover>) : (<span className="font-mono text-[9px] uppercase tracking-[0.2em]">
          Invalid
        </span>)}

      <button type="button" onClick={onRemove} aria-label={`Remove ${chip.email}`} className="flex size-5 items-center justify-center rounded-full opacity-50 hover:bg-foreground/[0.06] hover:opacity-100">
...
function CopyLinkButton() {
    ...
    return (<button type="button" onClick={() => {
      ...
      {copied() ? (<CheckIcon className="size-3.5 text-emerald-600 dark:text-emerald-400"/>) : (<LinkIcon className="size-3.5 opacity-60 group-hover:opacity-100"/>)}
