      ...
      <div className="mx-auto grid min-h-svh max-w-5xl grid-cols-[220px_1fr]">
        <aside className="border-r border-border/60 px-4 py-8">
          ...
          <ul className="mt-6 flex flex-col gap-0.5">
            {NAV.map(({ icon: Icon, label, active }) => (<li key={label}>
                <button type="button" className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-sm transition-colors ${active
            ...
            <li className="mt-3">
              <button type="button" className="flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-sm text-destructive transition-colors hover:bg-destructive/[0.06]">
        ...
        <main className="px-10 py-12 pb-32">
          ...
          <Section title="Identity" hint="Visible to anyone with access.">
            ...
            <Field label="Slug" htmlFor="ws-slug" hint="Used in URLs.">
              <div className="flex h-9 items-center gap-1 rounded-lg border border-input bg-background px-3 font-mono text-sm focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/24">
                ...
                <input id="ws-slug" value={slug()} onChange={(e) => {
          ...
          <Section title="Defaults">
            <SwitchRow title="Public sign-up" description="Anyone with a workspace email can request access." defaultChecked onChange={() => setDirty(true)}/>
            <SwitchRow title="Two-factor for admins" description="Require a second factor for owner and admin roles." defaultChecked onChange={() => setDirty(true)}/>
            <SwitchRow title="Show project IDs" description="Display the short ID next to project names everywhere." onChange={() => setDirty(true)}/>
          </Section>
        </main>
      </div>
...
function Section({ title, hint, children, }: {
    ...
    return (<section className="mt-8 grid grid-cols-[180px_minmax(0,1fr)] gap-x-10 gap-y-6">
      <div>
        ...
        {hint ? (<p className="mt-1 text-muted-foreground text-xs leading-relaxed">
            {hint}
          </p>) : null}
      </div>
...
function NativeSelect({ className, ...props }: React.SelectHTMLAttributes<HTMLSelectElement>) {
    return (<div className="relative">
      <select className={`h-9 w-full appearance-none rounded-lg border border-input bg-background pl-3 pr-9 text-sm outline-none transition-shadow focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/24 ${className ?? ""}`} {...props}/>
      <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden className="pointer-events-none absolute right-3 top-1/2 size-3.5 -translate-y-1/2 opacity-60">
        <polyline points="4 6 8 10 12 6"/>
      </svg>
    </div>);
}
function SwitchRow({ title, description, defaultChecked, onChange, }: {
    title: string;
    description: string;
