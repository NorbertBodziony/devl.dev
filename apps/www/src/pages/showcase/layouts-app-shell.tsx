        ...
        <aside className="flex w-64 flex-col overflow-y-auto border-r border-border/60 bg-foreground/[0.015]">
          ...
          <ul className="flex flex-col gap-0.5 px-2">
            ...
            return (<li key={item.key}>
                  <button type="button" onClick={() => setActiveItem(item.key)} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-sm transition-colors ${active
                    ? "bg-foreground/[0.06] text-foreground"
                    : "text-muted-foreground hover:bg-foreground/[0.04]"}`}>
                    <item.icon className="size-4 opacity-70"/>
                    <span className="flex-1 text-left">{item.label}</span>
                    {item.badge ? (<span className="rounded-full bg-foreground/10 px-1.5 py-0.5 font-mono text-[9px] text-foreground">
                        {item.badge}
                      </span>) : null}
                  </button>
                </li>);
          ...
          <ul className="flex flex-col gap-0.5 px-2">
            ...
            return (<li key={p.key}>
                  <button type="button" onClick={() => setActiveItem(p.key)} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-sm transition-colors ${active
                    ? "bg-foreground/[0.06] text-foreground"
                    : "text-muted-foreground hover:bg-foreground/[0.04]"}`}>
          ...
          <ul className="flex flex-col gap-0.5 px-2 pb-3">
            ...
            return (<li key={t.key}>
                  <button type="button" onClick={() => setActiveItem(t.key)} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-sm transition-colors ${active
                    ? "bg-foreground/[0.06] text-foreground"
                    : "text-muted-foreground hover:bg-foreground/[0.04]"}`}>
          ...
          <button type="button" className="mt-auto flex items-center gap-2 border-t border-border/60 px-3 py-2.5 text-left transition-colors hover:bg-foreground/[0.03]">
            <Avatar className="size-7">
              <AvatarFallback>SB</AvatarFallback>
            </Avatar>
          ...
          <div className="mt-6">
            ...
            <div className="rounded-xl border border-border bg-card">
              <ul className="divide-y divide-border/60">
                {ACTIVITY.map((a) => (<li key={a.name + a.time} className="flex items-center gap-3 px-4 py-3">
                    <Avatar className="size-7">
...
function SidebarSection({ label }: {
    ...
    return (<div className="px-3 pt-4 pb-1.5 font-mono text-[10px] text-muted-foreground tracking-[0.25em]">
      {label}
    </div>);
}
