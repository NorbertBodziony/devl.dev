...
export function EmptyInboxZeroShowcasePage() {
  ...
  return (
    <div class="relative h-dvh w-dvw overflow-hidden bg-background">
      <ParticleField
        src={dustyFieldSrc}
        sampleStep={2}
        threshold={48}
      ...
      <div class="absolute top-6 left-6 z-10 inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/60 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground backdrop-blur">
        <CheckCheckIcon class="size-3 text-foreground" aria-hidden />
        Inbox · 0 unread
      </div>

      <div class="absolute top-6 right-6 z-10 text-right font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">
        <div>This week</div>
        <div class="mt-0.5 font-heading text-xs tracking-normal text-foreground/85 normal-case">
          47 cleared
        </div>
      </div>

      <div class="absolute inset-x-0 bottom-0 z-10 flex flex-col items-center gap-4 px-6 pb-20 text-center">
        <div class="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">
          A quiet horizon
        </div>
        <h1 class="max-w-xl font-heading text-4xl leading-tight md:text-5xl">
          You're all caught up.
        </h1>
        <p class="max-w-md text-balance text-muted-foreground text-sm leading-relaxed">
