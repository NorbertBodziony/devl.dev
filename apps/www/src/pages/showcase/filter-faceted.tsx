...
export function FilterFacetedShowcasePage() {
    ...
    const filtered = createMemo(() => {
        return ISSUES.filter((iss) => {
            if (status().length && !status().includes(iss.status))
                return false;
            if (priority().length && !priority().includes(iss.priority))
                return false;
            if (assignee().length && !assignee().includes(iss.assignee))
                return false;
            if (label().length && !iss.labels.some((l) => label().includes(l)))
                return false;
            return true;
        });
    ...
    const counts = createMemo(() => {
        ...
        return {
            status: Object.fromEntries(STATUS_FACETS.map((f) => [f.value, count(f.value, (i) => i.status)])) as Record<StatusKey, number>,
            priority: Object.fromEntries(PRIORITY_FACETS.map((f) => [f.value, count(f.value, (i) => i.priority)])) as Record<PriorityKey, number>,
            assignee: Object.fromEntries(ASSIGNEE_FACETS.map((f) => [f.value, count(f.value, (i) => i.assignee)])) as Record<AssigneeKey, number>,
            label: Object.fromEntries(LABEL_FACETS.map((f) => [f.value, count(f.value, (i) => i.labels)])) as Record<LabelKey, number>,
        };
    }, []);
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="rounded-xl border bg-card p-4 shadow-xs/5">
          <div className="flex flex-wrap items-center gap-2">
            <FacetTrigger label="Status" selected={status()} labelFor={(v) => STATUS_FACETS.find((f) => f.value === v)?.label} onClear={() => setStatus([])}>
              <FacetCheckboxList value={status()} onValueChange={(v) => setStatus(v as StatusKey[])}>
                {STATUS_FACETS.map((f) => {
            const Icon = f.icon;
            return (<FacetRow key={f.value} value={f.value} label={f.label} count={counts().status[f.value]} leading={<Icon className="size-3.5 text-muted-foreground"/>}/>);
        })}
              </FacetCheckboxList>
            </FacetTrigger>

            <FacetTrigger label="Priority" selected={priority()} labelFor={(v) => PRIORITY_FACETS.find((f) => f.value === v)?.label} onClear={() => setPriority([])}>
              <FacetCheckboxList value={priority()} onValueChange={(v) => setPriority(v as PriorityKey[])}>
                {PRIORITY_FACETS.map((f) => (<FacetRow key={f.value} value={f.value} label={f.label} count={counts().priority[f.value]} leading={<PriorityDot value={f.value}/>}/>))}
              </FacetCheckboxList>
            </FacetTrigger>

            <FacetTrigger label="Assignee" icon={<UserIcon className="size-3.5"/>} selected={assignee()} labelFor={(v) => ASSIGNEE_FACETS.find((f) => f.value === v)?.label} onClear={() => setAssignee([])}>
              <FacetCheckboxList value={assignee()} onValueChange={(v) => setAssignee(v as AssigneeKey[])}>
                {ASSIGNEE_FACETS.map((f) => (<FacetRow key={f.value} value={f.value} label={f.label} count={counts().assignee[f.value]} leading={<span className={"flex size-5 items-center justify-center rounded-full font-medium text-[9px] " +
                    f.tone} aria-hidden>
                        {f.initials}
                      </span>}/>))}
              </FacetCheckboxList>
            ...
            <FacetTrigger label="Label" icon={<TagIcon className="size-3.5"/>} selected={label()} labelFor={(v) => LABEL_FACETS.find((f) => f.value === v)?.label} onClear={() => setLabel([])}>
              <FacetCheckboxList value={label()} onValueChange={(v) => setLabel(v as LabelKey[])}>
                {LABEL_FACETS.map((f) => (<FacetRow key={f.value} value={f.value} label={f.label} count={counts().label[f.value]} leading={<span className={"size-2 rounded-full " + f.color}/>}/>))}
              </FacetCheckboxList>
            </FacetTrigger>
            ...
            <Popover>
              <PopoverTrigger as={Button} variant="outline" size="sm" className="border-dashed">
                    <PlusCircleIcon />
                    Add filter
                  </PopoverTrigger>
              <PopoverPopup forceMount className="w-56 p-1">
...
function FacetTrigger({ label, icon, selected, labelFor, onClear, children, }: {
    ...
    return (<Popover>
      <PopoverTrigger as={Button} size="sm" variant="outline" className={count > 0
            ...
            {count > 0 ? (<>
                ...
                {count <= 2 ? (<span className="hidden gap-1 lg:inline-flex">
                    {selected.slice(0, 2).map((s) => (<Badge key={s} variant="secondary" size="sm" className="font-mono text-[10px]">
                        {labelFor(s) ?? s}
                      </Badge>))}
                  </span>) : null}
              </>) : null}
          </PopoverTrigger>
      <PopoverPopup forceMount className="w-64 p-0">
        <div className="flex flex-col">
          ...
          <div className="flex items-center justify-between p-1">
            <Button size="xs" variant="ghost" className="text-muted-foreground" onClick={onClear} disabled={count === 0}>
              Clear
            </Button>
...
function FacetSearch({ placeholder }: {
    placeholder: string;
}) {
...
function FacetRow({ value, label, count, leading, }: {
    value: string;
    label: string;
    count?: number;
    leading?: JSX.Element;
    ...
    return (<Label className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-accent">
      ...
      {leading ? (<span className="flex size-5 items-center justify-center">
          {leading}
        </span>) : null}
      <span className="flex-1 truncate">{label}</span>
      {count !== undefined ? (<span className="font-mono text-[10px] text-muted-foreground tabular-nums">
          {count}
        </span>) : null}
    </Label>);
}
function IssueList({ issues, onReset, }: {
    issues: Issue[];
    onReset: () => void;
}) {
    if (issues.length === 0) {
    ...
    return (<ul className="mt-6 divide-y rounded-xl border bg-card shadow-xs/5">
      {issues.map((iss) => (<li key={iss.id} className="flex items-center gap-3 px-4 py-3 first:rounded-t-xl last:rounded-b-xl hover:bg-background">
          ...
          <div className="hidden items-center gap-1.5 sm:flex">
            {iss.labels.map((l) => {
                ...
                if (!meta)
                    return null;
                return (<span key={l} className="inline-flex items-center gap-1 rounded-full border bg-background px-1.5 py-0.5 text-[10px]">
