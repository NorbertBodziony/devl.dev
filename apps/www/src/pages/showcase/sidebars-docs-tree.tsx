...
export function SidebarsDocsTreeShowcasePage() {
    ...
    return (<div className="grid min-h-svh grid-cols-[280px_1fr] bg-background text-foreground">
      <aside className="flex h-svh flex-col border-r border-border/60 bg-foreground/[0.02]">
        ...
        <nav className="flex-1 overflow-y-auto py-3">
          <ul>
            {TREE.map((doc) => (<TreeNode key={doc.id} doc={doc} depth={0} open={open()} onToggle={toggle} active={active()} onSelect={setActive}/>))}
          </ul>
        </nav>

        <div className="border-t border-border/60 px-3 py-2.5">
          <button type="button" className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-muted-foreground text-xs transition-colors hover:bg-foreground/[0.03] hover:text-foreground">
...
function TreeNode({ doc, depth, open, onToggle, active, onSelect, }: {
    doc: Doc;
    depth: number;
    ...
    return (<li>
      <button type="button" onClick={() => isFolder ? onToggle(doc.id) : onSelect(doc.id)} style={{ paddingLeft: `${depth * 14 + 12}px` }} className={`flex w-full items-center gap-1.5 py-1 pr-3 text-left text-sm transition-colors ${isActive
            ? "bg-foreground/[0.08] text-foreground"
            : "text-muted-foreground hover:bg-foreground/[0.03] hover:text-foreground"}`}>
        {isFolder ? (isOpen ? (<ChevronDownIcon className="size-3.5 shrink-0 opacity-60"/>) : (<ChevronRightIcon className="size-3.5 shrink-0 opacity-60"/>)) : (<span className="size-3.5 shrink-0"/>)}
        {isFolder ? (isOpen ? (<FolderOpenIcon className="size-3.5 shrink-0 opacity-70"/>) : (<FolderIcon className="size-3.5 shrink-0 opacity-70"/>)) : (<FileTextIcon className="size-3.5 shrink-0 opacity-60"/>)}
        <span className="truncate">{doc.title}</span>
      </button>
      {isFolder && isOpen && doc.children ? (<ul>
          {doc.children.map((child) => (<TreeNode key={child.id} doc={child} depth={depth + 1} open={open} onToggle={onToggle} active={active} onSelect={onSelect}/>))}
        </ul>) : null}
    </li>);
}
