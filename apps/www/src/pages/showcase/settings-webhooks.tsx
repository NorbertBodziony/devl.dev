...
const STATUS_TONE: Record<EndpointStatus, {
    ...
    success: {
        tone: "success",
        dot: "shadow-[0_0_0_2px_--theme(--color-emerald-500/15%)]",
        tip: "Healthy — last 100 deliveries succeeded",
    },
    failing: {
        tone: "danger",
        dot: "shadow-[0_0_0_2px_--theme(--color-red-500/15%)]",
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <header className="mb-8 flex items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <a href="#docs" className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-transparent px-[calc(--spacing(2.5)-1px)] font-medium text-muted-foreground text-sm transition-shadow hover:bg-accent hover:text-foreground">
              View docs
              <ArrowUpRightIcon />
            </a>
            <Button size="sm">
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-card px-4 py-3 shadow-xs/5">
          <div className="flex items-center gap-3">
            ...
            <div className="inline-flex items-center gap-1 rounded-md border bg-background px-2 py-1 font-mono text-xs">
              ...
              <CopyButton value={SECRET_REAL} variant="icon" label="Copy signing secret" className="size-5" iconClass="!size-3"/>
            </div>
          ...
          <Tooltip>
            <TooltipTrigger as="button" type="button" className="inline-flex items-center gap-1 text-amber-700 text-xs underline-offset-4 hover:underline dark:text-amber-400">
            ...
            return (<TableRow key={ep.id}>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap items-center gap-1">
                        ...
                        {overflow > 0 ? (<Tooltip>
                            <TooltipTrigger as="span" className="inline-flex cursor-default items-center gap-1 rounded-md border border-border bg-secondary px-2 py-0.5 font-medium text-muted-foreground text-[10px]">
                              {`+${overflow}`}
                            </TooltipTrigger>
                    ...
                    <TableCell>
                      <Tooltip>
                        <TooltipTrigger as="span" className="inline-flex cursor-default items-center gap-2">
                              <StatusIndicator tone={tone.tone} label={ep.status} dotClass={tone.dot}/>
                            </TooltipTrigger>
                    ...
                    <TableCell className="pe-4">
                      <Menu>
                        <MenuTrigger aria-label={`Actions for ${ep.url}`} className="inline-flex size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-accent hover:text-foreground">
...
function PayloadBlock({ label, body, status, }: {
    ...
    return (<CodeBlock value={body.trim()} label={label} copy preClass="max-h-56 overflow-auto" status={status !== undefined ? (<Badge variant="outline" size="sm" className={"font-mono text-[9px] " + statusTone(status)}>
              {status}
            </Badge>) : null}/>);
}
