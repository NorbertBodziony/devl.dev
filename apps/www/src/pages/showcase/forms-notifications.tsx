...
export function FormsNotificationsShowcasePage() {
    ...
    const flip = (key: string, channel: "email" | "push") => setPrefs((p) => ({
        ...p,
    ...
    return (<div className="min-h-svh bg-background py-16 text-foreground">
      <div className="mx-auto max-w-3xl px-6">
        ...
        {SECTIONS.map((section, idx) => (<section key={section.id} className="mt-8">
            <header>
            ...
            <ul className="mt-4 divide-y divide-border/50 rounded-xl border border-border/60 bg-background/40">
              {section.prefs.map((p) => {
                const v = prefs()[p.key]!;
                return (<li key={p.key} className="flex items-center gap-6 px-4 py-3.5">
