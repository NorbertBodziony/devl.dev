    ...
    <div className="min-h-svh bg-background px-10 py-10">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <SummaryTile label="Healthy" value={`${healthy}/${total}`} dotClass="bg-emerald-500" />
          <SummaryTile label="Degraded" value={`${degraded}`} dotClass="bg-amber-500" />
          <SummaryTile label="Down" value={`${down}`} dotClass="bg-rose-500" />
          <SummaryTile
            label="Total RPS"
    ...
    <div className="overflow-hidden rounded-xl border border-border/60 bg-background/40">
      <div className="flex items-start justify-between gap-3 px-4 pt-4">
        <div>
          ...
          <StatusIndicator tone={tone} label={s.health} className="mt-0.5" labelClass="font-mono tracking-[0.25em]" />
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 pt-4">
        ...
        <MetricTile
          ...
          tone={s.p99Ms > 500 ? "warning" : undefined}
          variant="compact"
        />
        <MetricTile
          label="errors"
          value={`${s.errorRate.toFixed(2)}%`}
          tone={s.errorRate > 1 ? "danger" : s.errorRate > 0.2 ? "warning" : undefined}
          variant="compact"
        />
      </div>
