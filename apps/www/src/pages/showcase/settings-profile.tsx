        ...
        <Section title="Identity">
          ...
          <Field label="Handle" htmlFor="p-handle">
            <div className="flex h-9 items-center gap-1 rounded-lg border border-input bg-background px-3 font-mono text-sm focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/24">
              ...
              <input id="p-handle" defaultValue="seancassiere" onChange={() => setDirty(true)} className="flex-1 bg-transparent outline-none"/>
            </div>
          </Field>
...
function NativeSelect({ className, icon, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & {
    icon?: React.ReactNode;
}) {
    return (<div className="relative">
      {icon ? (<span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2">
          {icon}
        </span>) : null}
      <select className={`h-9 w-full appearance-none rounded-lg border border-input bg-background ${icon ? "pl-8" : "pl-3"} pr-9 text-sm outline-none transition-shadow focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/24 ${className ?? ""}`} {...props}/>
