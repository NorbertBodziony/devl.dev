...
function Section({ title, icon, hint, children, }: {
    ...
    return (<section>
      <div className="flex items-center gap-2">
        {icon ? (<span className="inline-flex size-6 items-center justify-center rounded-md bg-foreground/[0.06]">
            {icon}
          </span>) : null}
...
function Field({ label, htmlFor, hint, children, }: {
    ...
    return (<div className="flex flex-col gap-2">
      {htmlFor ? (<Label htmlFor={htmlFor}>{label}</Label>) : (<span className="inline-flex font-medium text-foreground text-sm">
          {label}
        </span>)}
      {children}
