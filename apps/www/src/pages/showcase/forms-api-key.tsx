    ...
    return (<div className="flex min-h-svh items-center justify-center bg-background px-6 py-16 text-foreground">
      <form onSubmit={onSubmit} className="w-full max-w-lg rounded-2xl border border-border/60 bg-background/60 p-6 shadow-sm">
        ...
        <div className="mt-6 flex flex-col gap-1.5">
          ...
          <Input id="key-name" placeholder="e.g. CI deploy bot" value={name()} onChange={(e) => setName(e.target.value)} nativeInput/>
        ...
        <div className="mt-6">
          ...
          <div className="mt-3 flex flex-col gap-4">
            {SCOPES.map((group) => (<div key={group.group}>
                ...
                return (<li key={s.id}>
                        <label className={`flex cursor-pointer items-start gap-3 rounded-lg border px-3 py-2.5 transition-colors ${checked
                        ? s.destructive
                            ? "border-destructive/40 bg-destructive/[0.04]"
                            : "border-foreground/30 bg-foreground/[0.03]"
                        : "border-border/60 bg-background/40 hover:border-border"}`}>
                          <Checkbox checked={checked} onCheckedChange={() => toggleScope(s.id)} className="mt-0.5"/>
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className="font-mono text-[11px] text-foreground">
                                {s.id}
                              </span>
                              {s.destructive ? (<span className="rounded bg-destructive/10 px-1.5 py-0.5 font-mono text-[9px] text-destructive uppercase tracking-[0.2em]">
                                  Destructive
                                </span>) : null}
                            </div>
                            <p className="mt-0.5 text-muted-foreground text-xs">
...
function ExpirySelect({ value, onChange, }: {
    value: string;
    onChange: (v: string) => void;
}) {
    return (<div className="relative">
      <select id="key-expiry" value={value} onChange={(e) => onChange(e.target.value)} className="h-9 w-full appearance-none rounded-lg border border-input bg-background pl-3 pr-9 text-sm outline-none transition-shadow focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/24">
