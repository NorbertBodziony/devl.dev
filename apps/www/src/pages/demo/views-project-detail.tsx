...
export function ProjectDetailView() {
    ...
    return (<div className="mx-auto max-w-5xl">
      <button type="button" onClick={closeProject} className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em] transition-colors hover:bg-foreground/[0.05] hover:text-foreground">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => toggleStar(project.id)} aria-label="Star" className="rounded-md p-1 text-muted-foreground/40 transition-colors hover:text-amber-500">
                <StarIcon className={`size-4 ${project.starred ? "fill-amber-500 text-amber-500" : ""}`}/>
              </button>
            </div>
            {project.description ? (<p className="mt-1 max-w-2xl text-muted-foreground text-sm">
                {project.description}
              </p>) : null}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" type="button" onClick={() => pushToast({
            kind: "success",
            title: "Link copied",
          ...
          <Button size="sm" variant="ghost" type="button" onClick={() => requestArchive(project.id)}>
            <ArchiveIcon />
            Archive
          </Button>
        </div>
      ...
      <div className="mt-6 flex items-center gap-1 border-b border-border/60">
        {TABS.map((t) => (<button key={t.id} type="button" onClick={() => setTab(t.id)} className={`relative px-3 py-2.5 text-sm transition-colors ${tab() === t.id
                ? "text-foreground"
                : "text-muted-foreground hover:text-foreground"}`}>
            {t.label}
            {tab() === t.id ? (<span className="absolute right-0 bottom-0 left-0 h-px bg-foreground"/>) : null}
          </button>))}
      </div>
...
function Overview({ project, issues, }: {
      ...
      <div className="mt-6 grid grid-cols-1 gap-3 lg:grid-cols-[3fr_2fr]">
        <Card title="Recent issues">
          <ul className="mt-3 flex flex-col">
            {issues.slice(0, 5).map((i) => (<IssueRow key={i.id} issue={i}/>))}
            {issues.length === 0 ? (<li className="px-2 py-6 text-center font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em]">
                No issues yet.
              </li>) : null}
          </ul>
        </Card>
        ...
        <Card title="Members">
          <ul className="mt-3 flex flex-col gap-2">
            {[0, 1, 2, 3].map((i) => (<MemberRowMini key={i} index={i}/>))}
          </ul>
        </Card>
      </div>
    </>);
...
function Issues({ issues }: {
    ...
    return (<div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
      {groups.map((g) => {
            const items = issues.filter((i) => i.status === g.id);
            return (<div key={g.id} className="flex flex-col rounded-xl border border-border/60 bg-background/40">
            ...
            <ul className="flex flex-1 flex-col gap-2 p-2">
              {items.map((i) => (<li key={i.id} className="rounded-md border border-border/40 bg-background p-2.5 text-xs leading-snug">
              ...
              {items.length === 0 ? (<li className="px-2 py-4 text-center font-mono text-[10px] text-muted-foreground/70 uppercase tracking-[0.25em]">
                  Empty
                </li>) : null}
            </ul>
          </div>);
...
function Activity() {
    ...
    return (<div className="rounded-xl border border-border/60 bg-background/40 p-5">
      <ol className="relative ml-3 border-l border-border/60">
        {events.map((e, i) => (<li key={i} className="relative pl-6 pb-5 last:pb-0">
...
function IssueRow({ issue }: {
    ...
    return (<li className="group flex items-center gap-3 rounded-md px-2 py-2 transition-colors hover:bg-foreground/[0.03]">
...
function MemberRowMini({ index }: {
    ...
    if (!m)
        return null;
    return (<li className="flex items-center gap-2.5">
      ...
      <span className={`size-1.5 rounded-full ${m.status === "online" ? "bg-emerald-500" : m.status === "away" ? "bg-amber-500" : "bg-muted-foreground/40"}`}/>
    </li>);
}
function PriorityDot({ priority }: {
    ...
    return (<span className="inline-flex items-center gap-1" title={cfg.label}>
...
function Avatar({ id }: {
    ...
    if (!m)
        return null;
    return (<div className={`flex size-5 items-center justify-center rounded-full font-medium text-[9px] ${m.tone}`}>
      {m.initials}
    </div>);
}
