...
export function InboxView() {
    ...
    const visible = threads.filter((t) => {
        ...
        if (tab() === "unread")
            return t.unread;
        if (tab() === "mentions")
            return t.preview.includes("@") || t.subject.includes("@");
        return true;
    });
    const selected = threads.find((t) => t.id === selectedThreadId) ?? threads[0];
    return (<div className="grid h-full grid-cols-[340px_1fr]">
      <aside className="flex flex-col border-r border-border/60 bg-foreground/[0.015]">
        <div className="border-b border-border/60 px-4 py-3">
          ...
          <div className="mt-3 flex items-center gap-2 rounded-md border border-border/60 bg-background/60 px-2.5 py-1.5">
            ...
            <input value={query()} onChange={(e) => setQuery(e.target.value)} placeholder="Search threads…" className="flex-1 bg-transparent text-xs outline-none placeholder:text-muted-foreground"/>
          </div>
        </div>
        ...
        <ul className="flex-1 overflow-y-auto">
          {visible.map((t) => (<ThreadRow key={t.id} thread={t} active={t.id === selected?.id} onClick={() => selectThread(t.id)}/>))}
          {visible.length === 0 ? (<li className="px-6 py-12 text-center font-mono text-[10px] text-muted-foreground uppercase tracking-[0.25em]">
              Nothing here.
            </li>) : null}
        </ul>
      </aside>
...
function ThreadRow({ thread, active, onClick, }: {
    ...
    return (<li className={`relative cursor-pointer border-b border-border/40 px-4 py-3 transition-colors ${active ? "bg-foreground/[0.06]" : "hover:bg-foreground/[0.03]"}`} onClick={onClick}>
      {thread.unread ? (<span className="-translate-y-1/2 absolute top-1/2 left-1.5 size-1.5 rounded-full bg-primary"/>) : null}
      <div className="flex items-start gap-3">
        {author ? (<div className={`flex size-8 shrink-0 items-center justify-center rounded-full font-medium text-[11px] ${author.tone}`}>
            {author.initials}
          </div>) : null}
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center gap-1.5">
            {thread.pinned ? (<PinIcon className="size-3 shrink-0 text-amber-500"/>) : null}
    ...
    return (<>
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex items-center gap-1.5">
            <div className="flex items-center -space-x-1.5">
              {participants.map((p) => (<span key={p.id} className={`flex size-5 items-center justify-center rounded-full ring-2 ring-background font-medium text-[9px] ${p.tone}`}>
                  {p.initials}
                </span>))}
            </div>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto px-6 py-6">
        {thread.messages.map((m) => {
            ...
            if (!author)
                return null;
            return (<article key={m.id} className="flex items-start gap-3">
              <div className={`flex size-8 shrink-0 items-center justify-center rounded-full font-medium text-[12px] ${author.tone}`}>
                {author.initials}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                ...
                <div className="mt-2.5 flex items-center gap-1.5">
                  {(m.reactions ?? []).map((r) => (<button key={r.emoji} type="button" onClick={() => toggleReaction(thread.id, m.id, r.emoji)} className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-xs transition-colors ${r.mine
                        ? "border-foreground/30 bg-foreground/[0.06]"
                        : "border-border/60 bg-background/40 hover:border-border"}`}>
                      <span>{r.emoji}</span>
                      <span className="font-mono text-[10px]">{r.count}</span>
                    </button>))}
                  <button type="button" onClick={() => toggleReaction(thread.id, m.id, "👍")} aria-label="React" className="inline-flex size-5 items-center justify-center rounded-full border border-border/60 bg-background/40 text-muted-foreground transition-colors hover:border-border hover:text-foreground">
      ...
      <form onSubmit={onSubmit} className="border-t border-border/60 px-6 py-4">
        <div className="rounded-xl border border-border/60 bg-background/40 p-2 focus-within:border-foreground/40">
          <textarea value={draft()} onChange={(e) => setDraft(e.target.value)} rows={2} placeholder="Reply, or @mention someone…" className="w-full resize-none bg-transparent px-1 text-sm outline-none placeholder:text-muted-foreground" onKeyDown={(e) => {
...
function Tab({ children, active, onClick, }: {
    ...
    return (<button type="button" onClick={onClick} className={`rounded px-2 py-1 transition-colors ${active
